...
#' @param format One of `"skim"` or `"json"`.
...
#' @param dims For `format = "json"`, the number of rows and columns to show, 
#' as a numeric vector of length two. For example, the default 
#' `dims = c(5, 100)` shows the first 5 rows and 100 columns, whereas 
#' `dims = c(Inf, Inf)` would show all of the data.
  ...
  tool = function() {
    ellmer::tool(
      ...
      format = ellmer::type_string(
        paste(
          "The output format of the data frame: 'skim' or 'json'. Default 'skim'.",
          "",
