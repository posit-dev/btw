...
* Pass any R object to `btw()`, like `btw(mtcars)` or `btw(globalenv())`. Strings will be evaluated as R code and dispatched to the appropriate method unless they match a shortcut described below.
* `btw("{dplyr}")` will describe the dplyr package via the function reference and (if available) introductory vignette.
* `btw("?dplyr::across")` will return the help-file for `dplyr::across()`.
